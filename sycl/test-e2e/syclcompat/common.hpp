...
void call_for_each_impl(Func &&f, std::index_sequence<Is...>) {
  (f(std::integral_constant<std::size_t, Is>{}), ...);
}

template <typename Tuple, typename Func> void call_for_each(Func &&f) {
  call_for_each_impl<Tuple>(
      std::forward<Func>(f),
      std::make_index_sequence<std::tuple_size_v<Tuple>>{});
}

#define INSTANTIATE_ALL_TYPES(tuple, f)                                        \
  call_for_each<tuple>([](auto index) {                                        \
